...
export default function ConvertImageTool() {
  ...
  function addFiles(list) {
    setError('');
    const incoming = Array.from(list).filter((f) => /^image\/(jpeg|png|webp|gif|bmp)$/.test(f.type) || /\.(jpe?g|png|webp|gif|bmp)$/i.test(f.name));
    if (!incoming.length) {
  ...
  async function convertOne(item) {
    ...
    // JPG has no transparency, so fill white behind the image.
    if (target === 'jpeg') { ctx.fillStyle = '#ffffff'; ctx.fillRect(0, 0, canvas.width, canvas.height); }
  ...
  async function downloadZip() {
    ...
    const zip = new JSZip();
      ...
      const base = item.file.name.replace(/\.[^.]+$/, '');
      zip.file(`${base}.${ext}`, item.outBlob);
    });
    const blob = await zip.generateAsync({ type: 'blob' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `craftora-converted-${ext}.zip`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
  ...
  const doneCount = items.filter((i) => i.outBlob).length;
    ...
    <div>
      ...
      >
        <span className="w-16 h-16 rounded-2xl mx-auto flex items-center justify-center" style={{ background: 'var(--surface-soft)' }}>
          <Lucide.RefreshCw className="brand-text w-8 h-8" />
        </span>
        <h2 className="font-bold text-xl mt-5" style={{ color: 'var(--ink)' }}>Drop your images here</h2>
        <p className="muted text-sm mt-2">Or choose images to convert between JPG, PNG, and WebP.</p>
        <input ref={inputRef} type="file" accept="image/jpeg,image/png,image/webp,image/gif,image/bmp" multiple className="hidden" onChange={(e) => addFiles(e.target.files)} />
        ...
        <p className="muted text-xs mt-5">Your images stay private. Conversion happens in your browser and nothing is uploaded.</p>
      </div>
      ...
      {items.length > 0 && (
        <div className="mt-8">
          ...
          <div className="mb-5">
            <label className="block text-sm font-semibold mb-2" style={{ color: 'var(--ink)' }}>Convert to</label>
            <div className="flex gap-2">
              {FORMATS.map(([key, label]) => (
                <button key={key} onClick={() => setTarget(key)} className="rounded-lg px-4 py-2 text-sm font-semibold border surface" style={target === key ? { background: 'var(--brand)', color: '#fff', borderColor: 'var(--brand)' } : { color: 'var(--ink)' }}>{label}</button>
              ))}
            </div>
          </div>
              ...
              <div key={item.id} className="flex items-center gap-3 border surface rounded-xl p-3" style={{ background: 'var(--surface)' }}>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="muted text-xs">
                    ...
                    {item.status === 'done' && <> → <span className="mint-text font-bold">{ext.toUpperCase()} {formatSize(item.outSize)}</span></>}
                    {item.status === 'error' && <span style={{ color: '#b3261e' }}> · could not convert</span>}
                  </p>
                </div>
                {item.status === 'done' ? (
          ...
          <div className="flex flex-wrap gap-3 mt-6">
            <button onClick={convertAll} disabled={busy} className="rounded-xl px-6 py-3 font-bold text-sm inline-flex items-center justify-center gap-2 disabled:opacity-50" style={{ background: 'var(--brand)', color: '#fff' }}>
              {busy ? (<><Lucide.Loader2 className="w-4 h-4 animate-spin" /> Converting...</>) : (<><Lucide.RefreshCw className="w-4 h-4" /> Convert {items.length > 1 ? 'all' : 'image'} to {ext.toUpperCase()}</>)}
            </button>
            {doneCount > 1 && (
