  ...
  async function run() {
    ...
    try {
      ...
      if (doc.needsPassword()) {
        if (!password) {
          setError('This PDF is password protected. Enter its password to unlock it.');
          setBusy(false);
          return;
        }
      ...
      const buffer = doc.saveToBuffer('compress,decrypt');
      ...
      a.href = url;
      a.download = 'craftora-unlocked.pdf';
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
    } catch (err) {
      setError('Something went wrong while unlocking the PDF. Make sure the password is correct and the file is a valid PDF.');
    } finally {
      setBusy(false);
    }
  }
    ...
    <div>
      {!file && (
        ...
        >
          ...
          <p className="muted text-sm mt-2">Or choose a password protected PDF to unlock.</p>
          ...
          <p className="muted text-xs mt-5">Your file stays private. Unlocking happens in your browser and nothing is uploaded.</p>
        </div>
      )}
        ...
        <div>
          ...
          <div className="mt-6 max-w-md">
            ...
            <div className="relative">
              <input type={show ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter the PDF password" className="w-full rounded-xl border surface px-4 py-3 pr-11 text-sm bg-transparent" style={{ color: 'var(--ink)' }} />
            ...
            <p className="muted text-xs mt-2">Enter the password you already know. Craftora cannot open a PDF whose password you do not have.</p>
          </div>
          ...
          <button onClick={run} disabled={busy} className="w-full sm:w-auto rounded-xl px-6 py-3 font-bold text-sm mt-6 inline-flex items-center justify-center gap-2 disabled:opacity-50" style={{ background: 'var(--brand)', color: '#fff' }}>
            {busy ? (<><Lucide.Loader2 className="w-4 h-4 animate-spin" /> Unlocking...</>) : (<><Lucide.Unlock className="w-4 h-4" /> Unlock PDF and download</>)}
          </button>
        </div>
      )}
    </div>
