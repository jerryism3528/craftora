    ...
    <header className="sticky top-0 z-50 border-b surface" style={{ background: 'var(--surface)' }}>
      <div className="editorial-width px-4 sm:px-7 h-14 flex items-center gap-3">
        ...
        <nav className="hidden lg:flex items-center gap-1 ml-4">
          ...
          <div className="relative" ref={catRef}>
            <button
              ...
              className="px-3 py-1.5 text-sm font-semibold rounded-lg inline-flex items-center gap-1"
              style={{ color: 'var(--ink)' }}
            ...
            {catOpen && (
              <div
                className="absolute left-0 mt-2 w-64 rounded-2xl border surface shadow-card p-2 grid gap-0.5"
              ...
              >
                {categories.map((c) => (
                  <Link
                    ...
                    className="px-3 py-2 rounded-lg text-sm font-semibold hover:soft-surface"
                    style={{ color: 'var(--ink)' }}
                  >
                    {c.name}
          ...
          <Link href="/about" className="px-3 py-1.5 text-sm font-semibold rounded-lg hover:soft-surface" style={{ color: 'var(--ink)' }}>
            About
          </Link>
        </nav>
        ...
        <form onSubmit={submitSearch} className="ml-auto hidden md:block w-full max-w-xs">
          ...
          className="hidden md:inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-bold shrink-0"
      ...
      {mobileOpen && (
        <div className="lg:hidden border-t surface" style={{ background: 'var(--surface)' }}>
          <div className="editorial-width px-4 sm:px-7 py-4 space-y-4">
            <form onSubmit={submitSearch}>
              <div className="relative">
                ...
                <input
                  value={query}
                  onChange={(e) => setQuery(e.target.value)}
                  className="w-full rounded-lg border surface pl-9 pr-3 py-2 text-sm bg-transparent"
                  style={{ color: 'var(--ink)' }}
                  placeholder="Search tools"
                  aria-label="Search tools"
                />
            ...
            <Link href="/all-tools" onClick={() => setMobileOpen(false)} className="block py-1.5 text-sm font-semibold" style={{ color: 'var(--ink)' }}>
              All Tools
            </Link>
            ...
            <Link href="/about" onClick={() => setMobileOpen(false)} className="block py-1.5 text-sm font-semibold" style={{ color: 'var(--ink)' }}>
              About
            </Link>
